...
# extract the data
# ------------------------
if not (out_dir / "regional_error_metrics_one_month_forecast_admin.csv").exists:
...
if not (out_dir / "reference_l2_admin_gdf.csv").exists():
    ...
    d.analyze(da=da, selection="level_2", mean=True, save_data_fname=None)
    l2_gdf = d.gdf
    l2_gdf = l2_gdf[l2_gdf.datetime == l2_gdf.datetime.unique()[0]].drop(
        columns=["datetime", "mean_value"]
    )
...
l2_perf_gdf = d.join_dataframe_geodataframe(
    df=l2_perf, gdf=l2_gdf, gdf_colname="DISTNAME", df_colname="region_name"
