...
chirps = xr.open_dataset(data_dir / "interim" / "chirps_preprocessed" / "data_kenya.nc")
...
def plot_index(df: pd.DataFrame, index_name: str, title: str) -> None:
    fig, ax = plot_bar_anomalies(df, variable=index_name)
    ax.set_xlabel("Time")
...
plot_index(
    seasonal_metrics_df,
    index_name="AMO",
    title="Atlantic Multi-Decadal Oscillation Index",
)
