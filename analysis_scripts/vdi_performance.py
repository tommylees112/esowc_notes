...
index = VegetationDeficitIndex(out_dir / f'{model}_prediction_VHI.nc', resample_str=None)
index.fit('preds')
...
grouper.analyze(
    da=pred_VCI3M_da, mean=True, selection='level_2'
)
pred_gdf = (
    grouper.gdf
    .rename(columns={'mean_value': 'pred_value'})
    .drop(columns='DISTNAME')
)

# true
grouper = KenyaGroupbyRegion(data_dir)
grouper.analyze(
    da=true_VCI3M_da, mean=True, selection='level_2'
)
true_gdf = (
    grouper.gdf
    .rename(columns={'mean_value': 'true_value'})
    .drop(columns='DISTNAME')
)
...
plot_confusion_matrix(gdf.true_VDI, gdf.pred_VDI, classes=vals, normalize=True,
                      title='Normalized confusion matrix')
