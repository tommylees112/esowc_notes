...
def plot_spatial(da, title=None, product=None, savefig=True, **kwargs):
    ...
    if savefig:
        fig.savefig(
            png_dir
            / f'{product + "_" if product is not None else ""}{var.lower()}_mean_spatial_plot.png',
            dpi=300,
        )
        fig.savefig(
            svg_dir
            / f'{product + "_" if product is not None else ""}{var.lower()}_mean_spatial_plot.svg',
            dpi=300,
        )
...
def plot_ds_pixel(
    ds, ipixel, time_slice=("2008-01-01", "2014-12-31"), product=None, **kwargs
):
    plt.close("all")
    dims = [dim for dim in ds.dims]
    vars = [var for var in ds.variables if var not in dims]
    ...
    for var in vars:
        da = ds[var].sel(time=slice(time_slice[0], time_slice[1]))
        ...
        fig = plt.gcf()
        fig.savefig(
            png_dir / f"{product}{var.lower()}_time_series_P{ipixel}_plot.png", dpi=300
        )
        fig.savefig(
            svg_dir / f"{product}{var.lower()}_time_series_P{ipixel}_plot.svg", dpi=300
        )
