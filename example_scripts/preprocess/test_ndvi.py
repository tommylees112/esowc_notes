...
class TestNDVIPreprocessor:
    ...
    def _make_ndvi_dataset(
        ...
        dims = ["lat", "lon"]
        coords = {"lat": latitudes, "lon": longitudes, "ncrs": [1], "nv": [1]}
    ...
    def test_directories_created(tmp_path):
        ...
        assert (
            ...
            "Should have created a directory tmp_path/interim" "/ndvi_preprocessed"
        )

        assert (tmp_path / processor.preprocessed_folder / "ndvi_interim").exists(), (
            "Should have created a directory tmp_path/interim" "/ndvi_interim"
        )
    ...
    def test_preprocess(self, tmp_path):
        ...
        processor.preprocess(
            subset_str="kenya", regrid=regrid_path, parallel_processes=1
        )

        expected_out_path = tmp_path / "interim/ndvi_preprocessed" "/ndvi_kenya.nc"
        assert (
    ...
    def test_alternative_region(self, tmp_path):
        ...
        processor = NDVIPreprocessor(tmp_path)
        processor.preprocess(
            subset_str="ethiopia", regrid=regrid_path, parallel_processes=1
        )

        expected_out_path = tmp_path / "interim/ndvi_preprocessed" "/ndvi_ethiopia.nc"
        assert (
            expected_out_path.exists()
        ), f"Expected processed file to be saved to {expected_out_path}"
