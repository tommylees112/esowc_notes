...
class TestTrainingContinuation(unittest.TestCase):
    ...
    xgb_params_01 = {"verbosity": 0, "nthread": 1}
    ...
    def test_training_continuation(self):
        ...
        gbdt_02a = xgb.train(
            self.xgb_params_01, dtrain_2class, num_boost_round=10, xgb_model=gbdt_02
        )
        gbdt_02b = xgb.train(
            self.xgb_params_01,
            dtrain_2class,
        ...
        gbdt_03a = xgb.train(
            self.xgb_params_01, dtrain_2class, num_boost_round=7, xgb_model=gbdt_03
        )
        gbdt_03b = xgb.train(
            self.xgb_params_01,
            dtrain_2class,
        ...
        res2 = mean_squared_error(
            y_2class,
            gbdt_04.predict(dtrain_2class, ntree_limit=gbdt_04.best_ntree_limit),
        )
        assert res1 == res2

        gbdt_04 = xgb.train(
            self.xgb_params_02, dtrain_2class, num_boost_round=7, xgb_model=gbdt_04
        )
        assert (
            gbdt_04.best_ntree_limit
            == (gbdt_04.best_iteration + 1) * self.num_parallel_tree
        ...
        assert (
            gbdt_05.best_ntree_limit
            == (gbdt_05.best_iteration + 1) * self.num_parallel_tree
        )
        gbdt_05 = xgb.train(
            self.xgb_params_03, dtrain_5class, num_boost_round=3, xgb_model=gbdt_05
        )
        assert (
            gbdt_05.best_ntree_limit
            == (gbdt_05.best_iteration + 1) * self.num_parallel_tree
