...
engineer.engineer(
    test_year=1990, target_variable="VHI", pred_months=3, expected_length=3
)
...
train_ds = engineer._train_test_split(
    ds, years, target_variable="VHI", pred_months=3, expected_length=3
)
...
train_dict = engineer._stratify_training_data(
    train_ds, target_variable, pred_months, expected_length
)
...
train_ds = engineer._train_test_split(
    data, cast(List, test_year), target_variable, pred_months, expected_length
)
...
output_test_arrays: DDict[int, DDict[int, Dict[str, xr.Dataset]]] = defaultdict(
...
for year in years:
    for month in range(1, 13):
        if year > years[0] or month > 1:
            ...
            xy_test, _ = engineer.stratify_xy(
                ds, year, target_variable, month, pred_months, expected_length
            )
...
#
x = xr.open_dataset(
    "/Volumes/Lees_Extend/data/ecmwf_sowc/data/features/train/1985_12/x.nc"
)
y = xr.open_dataset(
    "/Volumes/Lees_Extend/data/ecmwf_sowc/data/features/train/1985_12/y.nc"
)
