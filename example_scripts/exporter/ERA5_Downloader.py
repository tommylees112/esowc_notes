#!/usr/bin/env python
#  https://retostauffer.org/data/ERA5_Downloader.py
# -------------------------------------------------------------------
# - NAME:        ERA5.py
...
if __name__ == "__main__":
    ...
    * https://cds.climate.copernicus.eu/cdsapp#!/dataset/reanalysis-era5-pressure-levels?tab=form
    ...
    latmax = 52.0
    the_subset = "/".join(
        ["{:.2f}".format(x) for x in [latmax, lonmin, latmin, lonmax]]
    )
    ...
    log.info("\n{:s}".format("".join(["-"] * 60)))
    log.info(
        "Processing data for years: {:s}".format(
            ", ".join([str(x) for x in options.years])
        )
    )
    ...
    for year in options.years:
        ...
        tmp = "ERA5_{:04d}{:s}_{:s}{:s}".format(
            year, "_test" if options.test else "", options.param, tmp
        )
