...
class DataArrayToGeoDataFrame:
    ...
    def __init__(self, country: str = "kenya", data_dir: Path = Path("data")) -> None:
    ...
    def get_values_for_region(
        ...
        return (
            df.explode("all_values").rename(columns={"all_values": "values"})
    ...
    def calculate_mean_per_region(
        ...
        assert region_da.shape == da.shape[1:], (
    ...
    def join_dataframe_geodataframe(
        df: pd.DataFrame, gdf: gpd.GeoDataFrame, gdf_colname: str, df_colname: str
    ) -> gpd.GeoDataFrame:
