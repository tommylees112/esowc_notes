...
class DataArrayToGeoDataFrame:
    """Convert an xr.DataArray object to a gpd.GeoDataFrame
    """
    def __init__(self,
                 country: str = 'kenya',
    ...
    @staticmethod
    def get_admin_level(selection: str) -> AdminBoundaries:
        level_1 = AdminBoundaries(
            ...
            nc_filepath=self.data_dir / 'analysis' / 'boundaries_preprocessed' / 'province_l1_kenya.nc',
        )
        ...
        level_2 = AdminBoundaries(
            ...
            nc_filepath=self.data_dir / 'analysis' / 'boundaries_preprocessed' / 'district_l2_kenya.nc',
        ...
        level_3 = AdminBoundaries(
            ...
            nc_filepath=self.data_dir / 'analysis' / 'boundaries_preprocessed' / 'division_l3.nc',
        )
        ...
        level_3_wards = AdminBoundaries(
            ...
            nc_filepath=self.data_dir / 'analysis' / 'boundaries_preprocessed' / 'level_3_wards.nc',
        )
        ...
        level_4 = AdminBoundaries(
            ...
            nc_filepath=self.data_dir / 'analysis' / 'boundaries_preprocessed' / 'location_l4.nc',
        )
        ...
        level_5 = AdminBoundaries(
            ...
            nc_filepath=self.data_dir / 'analysis' / 'boundaries_preprocessed' / 'sublocation_l5.nc',
        )
    ...
    def analyze(self, da: xr.DataArray,
                selection: str,
                mean: bool = True,
                save_data_fname: Optional[str] = None) -> gpd.GeoDataFrame:
        ...
        self.region_shp_path = self.admin_bound.region_shp_path
        assert self.region_shp_path.exists(), \
        'Have the AdminBoundaries Exporters been run?'

        self.region_data_path = self.admin_bound.region_data_path
        ...
        self.join_dataframe_geodataframe(
            df, df_colname='region_names',
    ...
    def calculate_mean_per_region(da: xr.DataArray,
        ...
        assert region_da.shape == da.shape[1:], 'Require matching shapes' \
    ...
    def join_dataframe_geodataframe(df: pd.DataFrame,
                                    ...
                                    gdf_colname: str,
                                    df_colname: str) -> gpd.GeoDataFrame:
        """Join pd.DataFrame with gpd.GeoDataFrame to produce
        a GeoDataFrame.
        """
