...
# ------ FUNCTIONS ------
def load_experiment_annual_scores(
    experiments: List[str] = ["one_month_forecast"],
    ...
    for experiment in experiments:
        if experiment == "one_month_forecast":
            ...
            models = ["rnn", "linear_regression", "linear_network", "ealstm"]
...
for model in ["previous_month", "linear_regression", "linear_network", "rnn", "ealstm"]:
    (
        monthly_scores.where(monthly_scores.metric == "rmse").plot(
            x="month", y=model, label=model, ax=ax
        )
    )
plt.legend()
