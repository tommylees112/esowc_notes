...
def get_performance_metrics(
    metric: str, score_dict: Dict[str, pd.DataFrame]
    ...
    for experiment, df in zip(score_dict.keys(), score_dict.values()):
        ...
        seasonality_of_error = (
            df.drop(columns=["year", "metric"]).groupby("month").mean().reset_index()
        )
...
r2_errors, r2_seasonality = get_performance_metrics(metric="r2", score_dict=score_dict)
