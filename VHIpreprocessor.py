...
class VHIPreprocessor(BasePreProcessor):
    ...
    def __init__(data_folder: Path = Path("data"), dataset: str = "vhi"):
    ...
    def _preprocess(
        ...
        * Save the output file to new folder
        ...
        print(f"** Done for {selected_vars}" f' {netcdf_filepath.split("/")[-1]} **')
    ...
    def preprocess(
        ...
        if n_parallel_requests > 1:  # Run in parallel:
            pool = multiprocessing.Pool(processes=n_parallel_requests)
            outputs = pool.map(
                partial(
    ...
    def create_new_dataset(
        ...
        if all_vars:
            ...
            assert selected_vars in list(ds.variables.keys()), (
                "Must" f" provide valid variables: {list(ds.variables.keys())}"
            )
...
class VCIPreprocessor(VHIPreprocessor):
    def __init__(data_folder: Path = Path("data")):
        self.dataset = "vci"
        super().__init__(data_folder)
