""" preprocessing utils contains functions for joining EXTERNAL datasests
...
def resample_time(ds, resample_str="M"):
    ...
    Example:
    -------
    resampled_time_data = ds.resample(time="M").first()
    """
    return ds.resample(time="M").first()
...
def convert_to_same_grid(reference_ds, ds, method="nearest_s2d"):
    ...
    >>> regrid_data = convert_to_same_grid(
            reference_ds, clean_data, method="nearest_s2d"
        )
    """
...
def select_same_time_slice(reference_ds, ds):
    ...
    Example:
    >>> correct_time_slice = select_same_time_slice(reference_ds, clean_data)
    """
